...
k.innerHeight()+2*x)})}var g=c("option",a),s="";if(a.is("[multiple]")){var v=navigator.userAgent.match(/Android/i)?!0:!1,I=navigator.userAgent.match(/(iPad|iPhone|iPod)/i)?!0:!1;v||I||d()}else h()};s();a.on("refresh",function(){a.off(".styler").parent().before(a).remove();s()})}});else if(a.is(":reset"))a.on("click",function(){setTimeout(function(){a.closest(e.wrapper).find("input, select").trigger("refresh")},1)})}).promise().done(function(){e.onFormStyled.call()})}});
		...
		function testSelDate(block) { // проверка даты из 3-x select на валидность и ограничение по возрасту(если необходимо)
			// block - <div class="field seldate validate" data-yearsrestrict="12">
			...
			if (dd.val()>-1 && mm.val()>-1 && yy.val()>-1) {
				...
				block.find('select').each(function(){
					...
					inp.removeClass('error').trigger('error',['off']);
					if (inp.val()=='-1') {
						inp.addClass('error').trigger('error',['on']);
					} else {
						inp.removeClass('error').trigger('error',['off']);
					}
				});
...
(function($) { //create closure
	$.fn.formValidator = function(options) {
		...
		this.each(function() {
			...
			function setError(inp,par,sel) {
				inp.removeClass('ok');
				inp.addClass('error').trigger('error',['on']);
				par.removeClass('ok');
				par.addClass('error');
				par.find('em.error'+sel).css('display', 'block');
			}
			function setOK(inp,par) {
				par.addClass('ok');
				inp.addClass('ok');
			}
			...
			inputs.removeClass('error').each(function() {
				var inp = $(this),
					par = inp.parents('.validate'),
					val
				if(form.data('simplesubmit')==true || inp.data('notag') == true ){
					 val = inp.val().replace( /<[^<|>]+?>/gi,'' ).replace( /\&[a-z]{4}\;/gi,'1' ).trim();
				}else{
					 val = inp.val();
				}
				...
				if (inp.data('validate') || inp.data('required')) {
					var oneof = false;
					if (inp.data('oneof')) { // одно из двух полей может быть не заполнено
						var idsel = $(inp.data('oneof'));
						if (idsel.length < 1) alert('не найдено поле для сравнения' + inp.attr('name'));
						if (val != '' || idsel.val() != '') {
							oneof = true;
						}
					}
					if ((inp.data('required') && ($.trim(inp.val()) == '')  || (inp.val()=='-1'&&inp.get(0).tagName=='SELECT') && !oneof))  {
						inp.addClass('error').trigger('error',['on']);
						par.addClass('error');
						par.find('em.error.required').css('display', 'block');
					} else {
						switch (inp.data('validate')) {
							case 'checkbox':
								if (!inp.prop('checked')) {
									setError(inp,par,'.validate');
								} else {
									setOK(inp,par);
								}
								break;
							case 'alpha':
								if (val != '') {
									var validchars = "qwertyuiopasdfghjklzxcvbnmйцукенгшщзхъфывапролджэячсмитьбю";
							...
							case 'phone':
								if (val != '') {
									...
									if (err == 0) {
										// 3. remove plus
										var noplusval = val.replace(/\+/g, '');
										if (val.length - noplusval.length > 1) {
											err++;
										} else {
											var summ = 0;
											for (var i = noplusval.length - 1; i >= 0; i--) {
												var x = new Number(noplusval.charAt(i));
												summ += x;
											}
											var medium = summ / noplusval.length;
											err++;
											for (var i = noplusval.length - 1; i >= 0; i--) {
												var x = new Number(noplusval.charAt(i));
												if (x != medium) {
						...
						if (inp.data('minlength') && !validateerror) {
							if (val != '') {
							if(form.data('simplesubmit')==true || inp.data('notag') == true){
								if (val.length+1 < inp.data('minlength') * 1) {
									setError(inp,par,'.minlength');
									validateerror = true;
								} else {
									setOK(inp,par);
								}
						...
						if (inp.data('maxlength') && !validateerror) {
							if (val != '') {
								if(form.data('simplesubmit')==true || inp.data('notag') == true){
								if (val.length+1 > inp.data('maxlength') * 1) {
									setError(inp,par,'.maxlength');
									validateerror = true;
								} else {
									setOK(inp,par);
								}
						...
						if (inp.data('identity') && !validateerror) {
							var idsel = $(inp.data('identity'));
							if (idsel.length < 1) alert('не найдено поле для сравнения' + inp.attr('name'));
							if (val != '' && idsel.val() != '' && val != idsel.val()) {
								setError(inp,par,'.identity');
								validateerror = true;
							}
						}
						if (!validateerror) {
